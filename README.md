...
One colour is handled separately: the chip's red-amber-green health tint, and the download trace, hero card and status pill that share it. That ramp is a reading rather than decoration — a link going bad is amber before it is red — so it stays a traffic light, but in the theme's own red, yellow and green. The shell surfaces neither green nor yellow, so they are read from the theme's `colors.toml`, by name or from the `color1` / `color2` / `color3` terminal slots.

Half the themes on a typical machine ship the right three hues at the wrong three saturations, so each stop is lifted first: it keeps the hue the theme chose and is raised to a readable chroma, with a wide lightness band that rescues a stop too dark or too pale to see. Stops already above the floor pass through untouched, and only a stop at genuinely zero chroma has no hue to preserve and borrows the built-in one. The three are then measured against each other as a weighted RGB distance, and a palette whose stops are really one colour falls back to the built-in ramp as a whole set: `blue-red-4k-warm`'s yellow `#e99b8c` and green `#ea9b8c` differ by a single step of red. Of the 40 themes installed here, 36 use their own palette.

This is the same ramp treatment CPU Pulse and RAM Pulse use, which is what makes the three read as siblings sitting beside each other on the bar. Everything that is merely chrome, including selected tabs, hover borders and the connected-network marker, uses the theme accent instead.
